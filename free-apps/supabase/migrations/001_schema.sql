-- Unified business suite: shared DB, per-user data via RLS
...
-- OTP storage (hashed codes from Edge Functions)
...
-- email_otps: no client access (service role only via Edge Functions)
...
create policy "ERP products own data"
  on public.erp_products for all
  using (auth.uid() = user_id or public.is_admin())
  with check (auth.uid() = user_id);

create policy "ERP orders own data"
  on public.erp_orders for all
  using (auth.uid() = user_id or public.is_admin())
  with check (auth.uid() = user_id);
...
create policy "POS products own data"
  on public.pos_products for all
  using (auth.uid() = user_id or public.is_admin())
  with check (auth.uid() = user_id);

create policy "POS sales own data"
...
create policy "CRM contacts own data"
  on public.crm_contacts for all
  using (auth.uid() = user_id or public.is_admin())
  with check (auth.uid() = user_id);

create policy "CRM deals own data"
  on public.crm_deals for all
  using (auth.uid() = user_id or public.is_admin())
  with check (auth.uid() = user_id);

-- Activity: users insert own; admins read all
...
create policy "Users insert own activity"
  on public.app_activity for insert
  with check (auth.uid() = user_id);

create policy "Users read own activity"
  on public.app_activity for select
  using (auth.uid() = user_id or public.is_admin());
