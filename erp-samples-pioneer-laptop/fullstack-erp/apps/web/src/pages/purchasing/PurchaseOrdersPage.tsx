...
export default function PurchaseOrdersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get('/purchasing/orders', { params: { status, page, limit: 20 } }).then(r => r.data),
  });
  ...
  return (
    <div className="p-6 animate-fade-in">
      <PageHeader title="Purchase Orders" description="Manage procurement and supplier orders">
        <Button size="sm"><Plus size={13} /> New PO</Button>
      </PageHeader>
      ...
      <Card>
        ...
        {isLoading ? <LoadingSpinner /> : orders.length === 0 ? (
          <EmptyState icon={<TruckIcon size={32} />} title="No purchase orders" description="Create your first purchase order" />
        ) : (
          <Table>
            <Thead>
              <Th>Reference</Th>
            ...
            <Tbody>
              {orders.map((o: any) => (
                <Tr key={o.id}>
                  ...
                  <Td className="text-muted-foreground">{o.expectedDate ? formatDate(o.expectedDate) : '—'}</Td>
